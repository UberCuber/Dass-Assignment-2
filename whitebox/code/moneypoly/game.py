""" 
Game module defines the Game class which manages the full state 
and flow of a moneypoly game session.
"""
...
class Game:
    ...
    def mortgage_property(self, player, prop):
        ...
        if payout == 0:
            ...
            return False
        disbursed = self.bank.pay_out(payout)
    ...
    def _apply_card(self, player, card):
        ...
        if action == "collect":
            ...
            self._card_action_move_to(player, value)
        elif action in ("birthday", "collect_from_all"):
            self._card_action_collect_from_all(player, value)

    def _card_action_move_to(self, player, value):
